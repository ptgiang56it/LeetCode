class Solution {
 public:
  int numberOfBeautifulIntegers(int low, int high, int k) {
    // dp[i][even][odd][remainder][k1][k2] := the number of beautiful integers,
    // considering the i-th digit with counts of even `even` digits and odd
    // `odd` digits, where the current number modulo k equals remainder, `k1`
    // indicates if the current digit is tightly bound for `low` and `k2`
    // indicates if the current digit is tightly bound for `high`
