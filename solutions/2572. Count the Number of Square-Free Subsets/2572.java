class Solution {
  public int squareFreeSubsets(int[] nums) {
    ...
    return (squareFreeSubsets(masks, 0, /*used=*/1) - 1 + kMod) % kMod;
  }
  ...
  private int squareFreeSubsets(int[] masks, int i, int used) {
    if (i == masks.length)
      return 1;
    if (dp[i][used] != null)
      return dp[i][used];
    final int pick = (masks[i] & used) == 0 ? squareFreeSubsets(masks, i + 1, used | masks[i]) : 0;
    final int skip = squareFreeSubsets(masks, i + 1, used);
    return dp[i][used] = (pick + skip) % kMod;
  }
